...
//    testThreadVisit();
  ...
  private Thread testVolatileOrder() {
    Thread thread= new Thread(new Runnable() {
      ...
      public void run() {
        while (!isinited) {
          LLogger.d(Thread.currentThread() + " 还没初始化");
          Thread.yield();
        }
        int v = number;
        if (v != 20) {
        ...
        LLogger.d("thread+" + Thread.currentThread() + " 访问到 number:" + number);
      }
    });
  ...
  public void run() {
    NUM = 3;
    LLogger.d("thread:" + Thread.currentThread() + " first num:" + NUM);
    NUM = 5;
    LLogger.d("thread:" + Thread.currentThread() + " second num:" + NUM);
  }
}
